import { LegacyMigrationConnectionManager } from './services/legacy-migration-connection-manager';
...
async function getPracticeMapping(connectionManager: LegacyMigrationConnectionManager): Promise<Map<number, string>> {
  ...
  const practiceResult = await connectionManager.queryLegacy<PracticeMappingRow>(practiceQuery);
...
async function migrateProfilesOnly(): Promise<void> {
  ...
  const connectionManager = new LegacyMigrationConnectionManager(
    ...
    {
      host: config.supabase.url.replace('https://', '').replace('.supabase.co', ''),
      port: 5432,
      database: 'postgres',
      username: 'postgres',
  ...
  try {
    ...
    const practiceMapping = await getPracticeMapping(connectionManager);
    ...
    const usersResult = await connectionManager.queryLegacy<LegacyUser>(usersQuery);
    const legacyUsers = usersResult.rows;
